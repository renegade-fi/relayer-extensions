...
use crate::crypto_mocks::{
    csprng::{CSPRNG_TERMINATION_ERROR, PoseidonCSPRNG, compute_poseidon_hash},
...
pub fn sample_identifier_seed(master_view_seed: Scalar, object_idx: usize) -> Scalar {
    ...
    identifier_seed_csprng.nth(object_idx).expect(CSPRNG_TERMINATION_ERROR)
}
...
pub fn sample_recovery_id(identifier_seed: Scalar, version: usize) -> Scalar {
    ...
    recovery_id_csprng.nth(version).expect(CSPRNG_TERMINATION_ERROR)
}
