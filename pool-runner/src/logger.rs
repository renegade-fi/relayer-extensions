...
//!
//! Every pool-runner log line should go through [`log_task!`] so it follows
//! the same `[<task>] [<outcome>] <description>` shape used by gardener,
//! funds-manager, and synthetic-tester. The envelope makes raw log output easy
//! to read at a glance and gives Datadog stable `@task` / `@outcome` attributes
...
//! and `synthetic-tester/src/logger.rs`):
//!
//! ```text
//! [<task>] [<outcome>] <description>  (+ task, outcome, and any extra fields)
//! ```
//!
//! - **Task** is a closed enum of operations pool-runner performs. New tasks
...
//!   field name `subject` for naming WHICH thing the log line is about
//!   (operation, pool, order) — that keeps dashboards aggregable across call
//!   sites that share a task.

use tracing::Level;

/// Closed vocabulary of operations pool-runner performs.
///
...
pub enum Task {
    /// Process-level lifecycle: boot, token-remaps load, config load,
    /// healthcheck server start, shutdown.
    ServiceLifecycle,
    /// Inbound HTTP request handling for the `/healthcheck` endpoint.
    HttpServer,
    /// Admin WebSocket subscription, fill dispatch, reconnect, and teardown.
    AdminWsListener,
    /// The reassignment loop: select pool → assign order → await fill →
    /// reassign back to global pool.
    PoolRouter,
    /// Fill-waiter registry operations: register, notify, remove, timeout.
    FillWaiter,
}
...
impl Task {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Task::AdminWsListener => "admin-ws-listener",
            Task::PoolRouter => "pool-router",
            Task::FillWaiter => "fill-waiter",
        }
    }
}
...
/// Emit a structured log line in the pool-runner taxonomy:
///
/// ```text
/// [<task>] [<outcome>] <description>     (+ task, outcome, and any extra fields)
/// ```
...
/// structured fields. Use the field name `subject` to name WHICH thing the
/// log line is about (pool, order, operation) so dashboards can aggregate
/// across tasks.
///
