//! Defines the application-specific logic for depositing funds into an existing
//! balance object.
...
/// A transition representing a deposit into an existing balance object
#[derive(Clone)]
pub struct DepositTransition {
    /// The now-spent nullifier of the balance being deposited into
    pub nullifier: Scalar,
    /// The block number in which the deposit was made
        ...
        conn.transaction(move |conn| {
            async move {
                ...
                if nullifier_processed {
                    warn!(
                        "Nullifier {nullifier} has already been processed, skipping deposit indexing"
                    );
...
#[cfg(test)]
mod tests {
    use crate::{db::test_utils::cleanup_test_db, state_transitions::{error::StateTransitionError, test_utils::{gen_create_balance_transition, gen_deposit_transition, setup_expected_state_object, setup_test_state_applicator, validate_balance_indexing}}};

    /// Test that a deposit is indexed correctly.
    #[tokio::test(flavor = "multi_thread")]
    async fn test_deposit() -> Result<(), StateTransitionError> {
