//! Defines the application-specific logic for paying the relayer fee accrued on
//! a balance object.
...
/// A transition representing the payment of the relayer fee accrued on a
/// balance object
#[derive(Clone)]
...
impl StateApplicator {
    ...
    pub async fn pay_relayer_fee(
        ...
        let PayRelayerFeeTransition { nullifier, block_number, new_relayer_fee_public_share } =
            transition;
...
mod tests {
    use crate::{
        ...
        state_transitions::{
            ...
            test_utils::{
                gen_deposit_new_balance_transition, gen_pay_relayer_fee_transition,
    ...
    async fn test_pay_relayer_fee() -> Result<(), StateTransitionError> {
        ...
        test_applicator
            .pay_relayer_fee(pay_fees_transition.clone(), false /* is_backfill */)
            .await?;
