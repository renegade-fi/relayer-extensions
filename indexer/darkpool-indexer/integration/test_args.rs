...
use darkpool_indexer::{db::test_utils::cleanup_test_db, indexer::Indexer, types::MasterViewSeed};
use eyre::Result;
...
impl TestArgs {
    ...
    pub async fn cleanup_test_db(&self) -> Result<()> {
        let test_context = self.expect_test_context();
        cleanup_test_db(&test_context.postgres).await?;
        Ok(())
    }
}
