...
use crate::{
    ...
    utils::{
        balance::{
            get_party0_first_balance, random_deposit, submit_deposit_new_balance, submit_withdrawal,
        },
...
async fn test_withdraw(mut args: TestArgs) -> Result<()> {
    ...
    let (receipt, nullifier) = submit_withdrawal(&args, &initial_balance).await?;
