...
fn calculate_quote_per_base_price(match_bundle: &AtomicMatchApiBundle) -> f64 {
    ...
    Token::from_addr(&quote.mint).convert_to_decimal(quote.amount)
        / Token::from_addr(&base.mint).convert_to_decimal(base.amount)
}
...
pub async fn record_external_match_metrics(
    ...
    record_endpoint_metrics(&base_mint, key_description.clone(), NUM_ATOMIC_MATCH_REQUESTS);

    let labels = vec![(KEY_DESCRIPTION_METRIC_TAG.to_string(), key_description)];

    // Get price
    let price = calculate_quote_per_base_price(&match_resp.match_bundle);
