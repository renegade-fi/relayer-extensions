...
use renegade_api::http::{
    external_match::{
        ...
        ExternalMatchRequest, ExternalOrder, ExternalQuoteRequest, ExternalQuoteResponse,
    },
...
use crate::{
    ...
    server::api_handlers::{external_match::RequestContext, rfqt::MatchBundleRef},
};
    ...
    match bundle_ref {
        MatchBundleRef::Malleable(bundle) => {
            let maker = bundle
                ...
                .ok_or_else(|| {
                    AuthServerError::serde("Missing maker address in settlement transaction")
                })?;
            let calldata =
                bundle.settlement_tx.input.input().cloned().ok_or_else(|| {
                    AuthServerError::serde("Missing settlement transaction input")
                })?;
