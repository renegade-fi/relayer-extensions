...
use crate::server::api_handlers::rfqt::{MatchBundleRef, RequestContextVariant};
...
impl Server {
    ...
    pub(crate) fn rfqt_post_request_malleable(
        ...
        let rfqt_response = transform_match_bundle_to_rfqt_response(
            &MatchBundleRef::Malleable(&external_match_resp.match_bundle),
            req,
        )?;
    ...
    pub(crate) fn rfqt_post_request_direct(
        ...
        let rfqt_response = transform_match_bundle_to_rfqt_response(
            &MatchBundleRef::Direct(&external_match_resp.match_bundle),
            req,
        )?;
