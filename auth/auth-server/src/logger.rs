...
//!
//! Every auth-server log line should go through [`log_task!`] so it
//! follows the same `[<task>] [<outcome>] <description>` shape used by
//! gardener and funds-manager. The envelope makes raw log output easy to
//! read at a glance and gives Datadog stable `@task` / `@outcome` attributes
//! for aggregation without text matching.
//!
//! Pattern (mirrors `gardener/src/utils/logger.ts` and
...
//! ```
//!
//! - **Task** is a closed enum of operations the auth-server performs.
...
//!   `key = value`. Reserve the field name `subject` for naming WHICH
//!   thing the log line is about (route, leg, key id) — that keeps
//!   dashboards aggregable across call sites that share a task.

use tracing::Level;

/// Closed vocabulary of operations the auth-server performs.
///
...
pub enum Task {
    ...
    HandleRejection,
    /// On-chain nonce / settlement-event listener.
    ChainEventListener,
    /// Generic request-proxy plumbing (forward + relay).
    ProxyRequest,
    /// Handling external-match quote requests (`/external-match/quote`).
    ExternalMatchQuote,
    /// Handling external-match assemble / match-bundle requests.
    ExternalMatchAssemble,
    /// Handling POST /rfqt/v3/quote: parse, forward, sponsor, assemble,
    /// transform response.
    RfqtQuote,
    /// Handling GET /rfqt/v3/levels: parse query, fetch market depths,
    /// transform to levels.
    RfqtLevels,
    /// Gas-sponsorship updates applied to quotes and match bundles.
    GasSponsorship,
    /// Quote / bundle / execution-cost rate limiters.
    RateLimit,
    /// Telemetry / metric-recording side paths.
    Telemetry,
...
impl Task {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Task::ChainEventListener => "chain-event-listener",
            Task::ProxyRequest => "proxy-request",
            Task::ExternalMatchQuote => "external-match-quote",
            Task::ExternalMatchAssemble => "external-match-assemble",
            Task::RfqtQuote => "rfqt-quote",
            Task::RfqtLevels => "rfqt-levels",
            Task::GasSponsorship => "gas-sponsorship",
            Task::RateLimit => "rate-limit",
...
/// funds-manager's set so dashboards aggregating across services see the
/// same labels.
///
/// Semantics:
...
// Reserved variants below (`Skipped`, `Partial`, `Retrying`, `Failed`) are
// part of the shared taxonomy with gardener and funds-manager. Allow them
// to live unused here so the auth-server stays speaking the same vocabulary
// when future tasks need those outcomes.
#[allow(dead_code)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Outcome {
    /// Work has begun.
...
impl Outcome {
    ...
    /// and funds-manager: successes / skips at INFO, partial / retrying at
    /// WARN, failed at ERROR. Call sites do not pick the level themselves —
    /// picking the right `Outcome` is enough.
...
/// Emit a structured log line in the auth-server taxonomy:
///
/// ```text
/// [<task>] [<outcome>] <description>     (+ task, outcome, and any extra fields)
/// ```
...
/// the log line is about (route, leg, key id) so dashboards can aggregate
/// across tasks.
///
